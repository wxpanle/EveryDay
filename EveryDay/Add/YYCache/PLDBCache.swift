...
extension UInt {
    var storageStyle: PLKVStorage.Style {
        if self == 0 {
            return .file
        } else if self == UInt.max {
            return .db
        } else {
            return .mixed
        }
...
func SharedApplication() -> UIApplication? {
    ...
    let _: () = {
        ...
        if nil == cls { isAppExtension = true }
        if cls?.responds(to: Selector.init(("sharedApplication"))) ?? false {
            isAppExtension = true
        }
    ...
    return UIApplication.perform(Selector.init(("sharedApplication")))?.takeRetainedValue() as? UIApplication
}
...
class PLKVStorage {
    ...
    /// 当前缓存类型
    public private(set) var style: Style = .mixed
    
    private var trashQueue: DispatchQueue?
    private var dbPath: String = ""
    private var dataPath: String = ""
    private var trashPath: String = ""
    
    private var db: OpaquePointer?
    private var dbStmtCache: [String: String] = [:]
    private var dbLastOpenErrorTime: TimeInterval = 0
    private var dbOpenErrorCount: UInt = 0
    
    class func storage(path: String, style: Style) -> PLKVStorage {
        return self.init(path: path, style: style)
    }
    
    required init(path: String, style: Style) {
        self.path = path
        self.style = style
    }
    ...
    func dbOpen() -> Bool {
        ...
        if sqlite3_open(dbPath.cString(using: .utf8), &db) == SQLITE_OK {
            ...
            db = nil
            dbLastOpenErrorTime = Date().timeIntervalSince1970
            dbOpenErrorCount += 1
            return false
        }
    }
